...
export function* generateUnmatchedTableFetch() {
  ...
  try {
  console.log('---------------------http://172.20.244.238:8000/ranging/npd_view/unmatchedprod?'+paramString+searchParams);
      // Table data
        const data = yield call(request,
          `http://172.20.244.238:8000/api/npd/unmatchedprod?`+paramString+searchParams);
        yield put(unmatchedProdTableSuccess(data));

   // Sku chart data
   //    const sku_chart = yield call(request,
   //    `http://172.20.244.238:8000/ranging/npd_view/psgskudistribution?`+paramString);
   //      yield put(skuChartSuccess(sku_chart));
   //
   //    // Table data
   //    const out_performance= yield call(request,
   //      `http://172.20.244.238:8000/ranging/npd_view/outperformance?`+paramString);
   ...
   //      `http://172.20.244.238:8000/ranging/npd_view/pricebucket?`+paramString);
...
try {
  ...
  console.log('---------------------http://172.20.244.238:8000/ranging/npd_view/psgskudistribution?'+paramString);

   // Sku chart data
      const sku_chart = yield call(request,
      `http://172.20.244.238:8000/api/npd/psgskudistribution?`+paramString);
        yield put(skuChartSuccess(sku_chart));
...
export function* generateOutPerformanceChartFetch() {
  ...
  console.log('---------------------http://172.20.244.238:8000/ranging/npd_view/outperformance?'+paramString);
  ...
  try {
      ...
      const out_performance= yield call(request,
        `http://172.20.244.238:8000/api/npd/outperformance?`+paramString);
...
try {
  ...
  console.log('---------------------http://172.20.244.238:8000/ranging/npd_view/pricebucket?'+paramString);

      // Price gravity chart data

      const price_gravity = yield call(request,
        `http://172.20.244.238:8000/api/npd/pricebucket?`+paramString);
      yield put(priceGravitySuccess(price_gravity));
...
export function* generateSideFilter() {
  try {
    ...
    console.log('http://172.20.244.238:8000/ranging/npd_view1/filter_data?'+urlParams);

    // if (urlParams==='')
    // {urlParams='default'
    // }

    const data = yield call(request, `http://172.20.244.238:8000/api/npd_view1/filter_data?` + urlParams);


    // const data = yield call(request, `http://localhost:8090/wash/?format=json`);
    // const data = yield call(request, `http://172.20.244.238:8000/ranging/npd_view/filter_data?`);
